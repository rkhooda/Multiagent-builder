...
def devops_agent(state: dict) -> dict:
    ...
    Generates the active profile's devops file set regardless of the task plan,
    since every project of that shape needs these for deployment.
    """
    ...
    # Devops file set + prompt come from the active profile (Improvement 03).
    # A profile that declares no devops phase generates nothing here; the
    # fence-cleanup and artifact-validation passes below still run.
    profile = active_profile(state)
    devops_specs = list(profile.devops_files) if profile.phase("devops") else []
    system_prompt = profile.prompt_for("devops") if devops_specs else ""
    ...
    for i, file_spec in enumerate(devops_specs):
        ...
        try:
            ...
            if result["success"]:
                ...
                manager.broadcast_sync(project_id, {
                    ...
                    "filepath": filepath,
                    "phase": "devops",
                    "progress": f"{i+1}/{len(devops_specs)}"
                })
            else:
                files_failed += 1
