...
**Pipeline shape** (`backend/app/graph/pipeline.py`): `research → requirements → human_gate_1 → architecture → human_gate_2 → planning → human_gate_3 → frontend_code → backend_code → database → qa → devops → human_gate_4 → END`. Gate 1 and 2 support `edit`/`reject` routing back to an earlier stage or to `cancelled`; gate 3/4 are approve-or-reject only today.
