...
5. **Are loading, error and empty states handled?** Only for a component that fetches. Missing error handling on a component that fetches is `major`; missing empty state is `minor`. A purely presentational section that fetches nothing needs none of them — do not ask for them.
